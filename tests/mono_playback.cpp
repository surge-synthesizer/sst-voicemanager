...
TEST_CASE("Mono Mode - Two Layers (Duophonic)")
{
    ...
    vm.setPlaymode(1477, vm_t::PlayMode::MONO_NOTES, vm_t::MonoBehavior::MONO_RETRIGGER);
    vm.setPlaymode(1832, vm_t::PlayMode::MONO_NOTES, vm_t::MonoBehavior::MONO_RETRIGGER);
...
TEST_CASE("Mono Mode - Sustain Pedal")
{
    ...
    SECTION("Release with Gated When Releasing")
    {
        ...
        INFO("Release the pedal, release the voices");
        vm.updateSustainPedal(0, 0, 0);
        REQUIRE_VOICE_COUNTS(1, 0);
        REQUIRE_VOICE_MATCH(1, v.key() == 60);
        tp.processFor(20);
    ...
    SECTION("Release when non-gated but pedal held (surge 6620)")
    {
        ...
        INFO("The release of the pedal releases us");
        vm.updateSustainPedal(0, 0, 0);
        REQUIRE_VOICE_COUNTS(1, 0);
        REQUIRE_VOICE_MATCH(1, v.key() == 62);

        INFO("And then processing silences us");
        tp.processFor(20);
        REQUIRE_NO_VOICES
    }
}
...
TEST_CASE("Mono Mode - Two Layers, One Poly")
{
    SECTION("Case one")
    {
        ...
        vm.setPlaymode(8675309, vm_t::PlayMode::MONO_NOTES, vm_t::MonoBehavior::MONO_RETRIGGER);
...
TEST_CASE("Mono Mode - Poly and Mono on same key with multi-voice start")
{
    ...
    vm.setPlaymode(2112, vm_t::PlayMode::MONO_NOTES, vm_t::MonoBehavior::MONO_RETRIGGER);
